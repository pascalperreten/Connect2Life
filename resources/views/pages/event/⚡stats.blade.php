...
new class extends Component {
    ...
    public function mount(Ministry $ministry, Event $event)
    {
        ...
        $this->event = $event;
        $this->decisions_without_contact_details = $this->event->decisions_without_contact_details;
        ...
        $this->contacts_data = [['name' => __('With decision') . ' (' . $this->contacts_decided . ')', 'value' => $this->contacts_decided], ['name' => __('Without decision') . ' (' . $this->contacts_not_decided . ')', 'value' => $this->contacts_not_decided], ['name' => __('Contacted') . ' (' . $this->contacted . ')', 'value' => $this->contacted], ['name' => __('Met') . ' (' . $this->met . ')', 'value' => $this->met], ['name' => __('Part of Church') . ' (' . $this->part_of_church . ')', 'value' => $this->part_of_church], ['name' => __('Not Interested') . ' (' . $this->not_interested . ')', 'value' => $this->not_interested], ['name' => __('Invalid Contact Details') . ' (' . $this->invalid_contact_details . ')', 'value' => $this->invalid_contact_details]];
    }
};
?>
...
<div class="space-y-6">
    <div class="space-y-4">
        ...
        <flux:heading size="xl">{{ $this->event->name }}</flux:heading>
    ...
    <livewire:event-nav :ministry="$this->ministry" :event="$this->event">
        <div class="space-y-6">
            <flux:card>
                ...
                <flux:chart wire:model="decisions_data" class="aspect-[1/1] w-[40%] md:aspect-[2/1] lg:aspect-[3/1]">
                    <flux:chart.svg>
            ...
            <flux:chart wire:model="contacts_data" class="aspect-[1/1] md:aspect-[2/1] lg:aspect-[3/1]">
                <flux:chart.svg>
