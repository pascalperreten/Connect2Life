<div>
    ...
    <div>
        @if ($this->ministry->members->isEmpty())
            <flux:text>No members have been added to this crusade yet.</flux:text>
        @else
                    ...
                    <flux:table.rows>
                        @foreach ($ministry->members as $member)
                            <flux:table.row :key="$member->id">
                                <flux:table.cell>{{ $member->first_name . ' ' . $member->last_name }}</flux:table.cell>
                                <flux:table.cell>
                                    {{ $member->role }}
                                </flux:table.cell>
                                <flux:table.cell class="text-end" inset="top-bottom">
                                    <flux:button icon:trailing="arrow-up-right" href="/" size="sm"
                                        wire:navigate>edit
                                    </flux:button>
                                </flux:table.cell>
                            </flux:table.row>
    ...
    <div>
        ...
        <flux:button href="https://fluxui.dev/components/button" variant="ghost" class="mt-4" wire:navigate>Neue
            Crusade erstellen
        </flux:button><br>
